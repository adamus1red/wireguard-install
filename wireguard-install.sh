...
PresharedKey = $psk
AllowedIPs = 172.17.0.$octet/32$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/128")
# END_PEER $client
EOF
	# Create client configuration
	cat << EOF > ~/"$client".conf
[Interface]
Address = 172.17.0.$octet/24$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/64")
DNS = $dns
PrivateKey = $key
...
[Interface]
Address = 172.17.0.1/24$([[ -n "$ip6" ]] && echo ", fddd:2c4:2c4:2c4::1/64")
...
EOF
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd --zone=trusted --add-source=172.17.0.0/24
		firewall-cmd --permanent --add-port="$port"/udp
		firewall-cmd --permanent --zone=trusted --add-source=172.17.0.0/24
		# Set NAT for the VPN subnet
		firewall-cmd --direct --add-rule ipv4 nat POSTROUTING 0 -s 172.17.0.0/24 ! -d 172.17.0.0/24 -j SNAT --to "$ip"
		firewall-cmd --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s 172.17.0.0/24 ! -d 172.17.0.0/24 -j SNAT --to "$ip"
		if [[ -n "$ip6" ]]; then
...
[Service]
Type=oneshot
ExecStart=$iptables_path -t nat -A POSTROUTING -s 172.17.0.0/24 ! -d 172.17.0.0/24 -j SNAT --to $ip
ExecStart=$iptables_path -I INPUT -p udp --dport $port -j ACCEPT
ExecStart=$iptables_path -I FORWARD -s 172.17.0.0/24 -j ACCEPT
ExecStart=$iptables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -t nat -D POSTROUTING -s 172.17.0.0/24 ! -d 172.17.0.0/24 -j SNAT --to $ip
ExecStop=$iptables_path -D INPUT -p udp --dport $port -j ACCEPT
ExecStop=$iptables_path -D FORWARD -s 172.17.0.0/24 -j ACCEPT
	...
	case "$option" in
		...
		3)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				if systemctl is-active --quiet firewalld.service; then
					ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | grep '\-s 172.17.0.0/24 '"'"'!'"'"' -d 172.17.0.0/24' | grep -oE '[^ ]+$')
					# Using both permanent and not permanent rules to avoid a firewalld reload.
					firewall-cmd --remove-port="$port"/udp
					firewall-cmd --zone=trusted --remove-source=172.17.0.0/24
					firewall-cmd --permanent --remove-port="$port"/udp
					firewall-cmd --permanent --zone=trusted --remove-source=172.17.0.0/24
					firewall-cmd --direct --remove-rule ipv4 nat POSTROUTING 0 -s 172.17.0.0/24 ! -d 172.17.0.0/24 -j SNAT --to "$ip"
					firewall-cmd --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s 172.17.0.0/24 ! -d 172.17.0.0/24 -j SNAT --to "$ip"
